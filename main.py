...
def find_station(x: int, y: int) -> 'Station':
	return next((station for station in Stations if station.x == x and station.y == y), None)
	...
	while True:
		...
		if station.depth == len(_trains):
			...
			for train in _trains:
				...
				if len(station.no_go) > 0: # TODO: now picks up trains that are underway
...
if __name__ == "__main__":
	...
	random.seed(421)
	t = [Train(x=33,y=33,id="Ic01")] + [Train(x=randint(1,20),y=randint(1,20),id=f"Spr{_}") for _ in range(9)]
